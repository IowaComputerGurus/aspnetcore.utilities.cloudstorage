...
namespace ICG.AspNetCore.Utilities.CloudStorage
{
    ...
    public interface IAzureCloudStorageProvider
    {
        ...
        /// </summary>
        /// <example>
        /// Uploading with slugContent = "My Test Slug" a file named "text.pdf" will result in the file uploading as my-test-slug.pdf.  Taking the slug and adding the proper file extension.
        /// </example>
    ...
    public class AzureCloudStorageProvider : IAzureCloudStorageProvider
    {
        ...
        public AzureCloudStorageProvider(IOptions<AzureCloudStorageOptions> storageOptions, IUrlSlugGenerator urlSlugGenerator, IMimeTypeMapper mimeTypeMapper)
        {
        ...
        public async Task<string> StoreObject(string desiredContainer, Stream fileContents, string desiredName)
        {
            ...
            var container = blobClient.GetContainerReference(desiredContainer.ToString().ToLower());
            ...
            if (_mimeTypeMapper.TryGetMimeType(desiredName, out string contentType))
                blockBlob.Properties.ContentType = contentType;

            //Upload it
            await blockBlob.UploadFromStreamAsync(fileContents);

            return $"{_storageOptions.Value.RootClientPath}/{desiredContainer.ToString().ToLower()}/{desiredName}";
        }
        ...
        public async Task<bool> DeleteObject(string expectedContainer, string fullObjectUrl)
        {
            ...
            //Create the client & container reference
            var blobClient = storageAccount.CreateCloudBlobClient();
            var container = blobClient.GetContainerReference(expectedContainer.ToString().ToLower());
